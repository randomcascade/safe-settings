...
class Settings {
  ...
  static async sync (nop, context, repo, config, ref) {
    ...
    try {
      ...
      const repoDetails = await context.github.repos.get({ owner: repo.owner, repo: repo.repo })
      if(repoDetails.data.archived) {
        context.log.debug(`Skipping repo: ${repo.repo} as it is archived`)
        return
      }
  ...
  async eachRepositoryRepos (github, log) {
    ...
    return github.paginate('GET /installation/repositories').then(repositories => {
      return Promise.all(repositories.map(async repository => {
        ...
        const repoDetails = await github.repos.get({ owner: repository.owner.login, repo: repository.name })
        if(repoDetails.data.archived) {
          log.debug(`Skipping repo: ${repository.name} as it is archived`)
          return null
        }
